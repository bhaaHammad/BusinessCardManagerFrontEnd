...
export const mockHttpInterceptor: HttpInterceptorFn = (req, next) => {
  ...
  if (req.method === 'GET' && req.url.includes('/api/cards') && !req.url.includes('/export')) {
    ...
    return of(new HttpResponse({ status: 200, body: filteredCards })).pipe(delay(300));
  }

  if (req.method === 'POST' && req.url.includes('/api/cards') && !req.url.includes('/import') && !req.url.includes('/export')) {
    const body = req.body as BusinessCard;
    const newCard: BusinessCard = {
      ...body,
      id: Date.now().toString(),
    };
    MOCK_BUSINESS_CARDS.push(newCard);
    return of(new HttpResponse({ status: 201, body: newCard })).pipe(delay(300));
  }

  if (req.method === 'DELETE' && req.url.includes('/api/cards/')) {
  ...
  if (req.method === 'POST' && req.url.includes('/api/cards/import/commit')) {
    const cards = (req.body as { cards: BusinessCard[] }).cards;
    cards.forEach((card) => {
      card.id = Date.now().toString() + Math.random().toString(36).substr(2, 9);
      MOCK_BUSINESS_CARDS.push(card);
    });
